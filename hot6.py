...
def subscription(message):
    ...
    print(f'\033[1;97m [•] YOU NEED  APPROVAL')
    line()
...
def generate_ua():
    fb_versions = [
        "404.0.0.10.120", "401.0.0.29.97", "390.0.0.33.105",
        "381.0.0.9.119", "370.0.0.8.118"
    ]

    densities = ["1.5", "2.0", "2.5", "3.0"]
    langs = ["en_US", "en_GB", "en_NG", "fr_FR", "es_ES"]
    carriers = ["Generic", "MTN", "Airtel", "GLO", "Vodafone", "Orange"]
    android_versions = ["9", "10", "11", "12", "13", "14"]

    android_devices = [
        ("Samsung", "Samsung", "SM-G998B", "1080", "2400"),
        ("Samsung", "Samsung", "SM-A146P", "720", "1600"),
        ("Infinix", "Infinix", "X688B", "720", "1560"),
        ("TECNO", "TECNO", "K7", "480", "854"),
        ("Xiaomi", "Xiaomi", "M2010J19SG", "1080", "2340"),
    ]
    ...
    # --- Android Facebook Standard ---
    for mf, bd, dv, w, h in android_devices:
        fb_formats.append(
            f"FBAN/FB4A;FBAV/{random.choice(fb_versions)};FBBV/{random.randint(250000000,360000000)};"
            f"FBDM={{density={random.choice(densities)},width={w},height={h}}};"
            f"FBLC/{random.choice(langs)};FBRV/{random.randint(200000000,300000000)};"
            f"FBCR/{random.choice(carriers)};FBMF/{mf};FBBD/{bd};FBPN/com.facebook.katana;"
            f"FBDV/{dv};FBSV/{random.choice(android_versions)};FBCA/arm64-v8a"
        )

    # --- Facebook Lite ---
    for mf, bd, dv, w, h in android_devices:
        fb_formats.append(
            f"FBAN/FBLITE;FBAV/{random.choice(fb_versions)};FBBV/{random.randint(15000000,60000000)};"
            f"FBDM={{density={random.choice(densities)},width={w},height={h}}};"
            f"FBLC/{random.choice(langs)};FBRV/{random.randint(150000000,260000000)};"
            f"FBCR/{random.choice(carriers)};FBMF/{mf};FBBD/{bd};FBPN/com.facebook.lite;"
            f"FBDV/{dv};FBSV/{random.choice(android_versions)};FBCA/armeabi-v7a"
        )

    # --- Messenger Android ---
    for mf, bd, dv, w, h in android_devices:
        fb_formats.append(
            f"FBAN/MESSENGER;FBAV/{random.choice(fb_versions)};FBBV/{random.randint(250000000,360000000)};"
            f"FBDM={{density={random.choice(densities)},width={w},height={h}}};"
            f"FBLC/{random.choice(langs)};FBPN/com.facebook.orca;"
            f"FBDV/{dv};FBSV/{random.choice(android_versions)};FBCA/arm64-v8a"
        )
    ...
    fb_formats += [
        (
            f"FBAN/FBIOS;FBAV/{random.choice(fb_versions)};FBBV/{random.randint(350000000,450000000)};"
            f"FBDV/{dv};FBSV/{random.choice(['15.7', '16.5', '17.4'])};FBPR/Apple;"
            f"FBPN/com.facebook.Facebook;FBDM={{scale=3.0,width=1179,height=2556}};"
            f"FBLC/{random.choice(langs)}"
        )
        for dv in ios_devices
    ]

    return random.choice(fb_formats)
...
logo = f'''
    ...
    {gx}╚██████╔╝{wx}███████╗{gx}██║░╚███║
    {gx}░╚═════╝{wx}░╚══════╝{gx}╚═╝░░╚══╝ {bx}|{gx} VERSION  {bx}:{wx} 0.9
{wx}{47*'-'}
